function [cell_struct, face_struct, vertices, cells] = buildPolyGrid(domain, n_cells)
 
    % number of Lloyd iteration
    maxIter = 30; 
    [vertices, cells, ~, ~, cell_centers] = PolyMesher(domain, n_cells, maxIter);
    
    % FACE STORAGE: center, faces, volume, normal
    face_map = containers.Map();
    face_count = 0;
    face_struct = struct('nodes', {}, 'cells', {}, 'center', {}, 'normal', {}, 'area', {});
    cell_struct = struct('center', {}, 'faces', {}, 'faces_orientation', {}, 'volume', {});
    
    for c = 1:n_cells
        verts = cells{c}; % vertex indices (global) of cell c
        n_verts = length(verts);
        
        for i = 1:n_verts
            v1 = verts(i);
            v2 = verts(mod(i, n_verts) + 1); % wrap around
    
            % Enforce ordering so that v1 < v2
            if v2 < v1
                tmp = v1;
                v1 = v2;
                v2 = tmp;
                reversed = true;
            else
                reversed = false;
            end
            
            % Unique key for the edge
            key1 = sprintf('%d_%d', v1, v2);
            
            if isKey(face_map, key1)
                f_id = face_map(key1);
                face_struct(f_id).cells(end+1) = c;
            else
                face_count = face_count + 1;
                face_map(key1) = face_count;
    
                % build new face properties
                x1 = vertices(v1,:);
                x2 = vertices(v2,:);
                center = 0.5 * (x1 + x2);
                edge_vec = x2 - x1;
                
                % Normal vector is 90 deg CCW rotation of edge vector
                normal = [edge_vec(2), -edge_vec(1)];
                normal = normal / norm(normal);
                
                % If edge was reversed, flip normal direction to keep
                % consistent orientation from original v1->v2 ordering
                if reversed
                    normal = -normal;
                end
                
                area = norm(edge_vec);
    
                face_struct(face_count).nodes = [v1, v2];
                face_struct(face_count).cells = [c];
                face_struct(face_count).center = center(:);
                face_struct(face_count).normal = normal(:);
                face_struct(face_count).area = area;
            end
        end
    end
    
    % CELL STORAGE: center, faces, faces_orientation, volume
    for c = 1:n_cells
        verts = cells{c};
        n_verts = length(verts);
    
        cell_struct(c).center = cell_centers(c,:)';
        cell_struct(c).volume = polyarea(vertices(verts,1), vertices(verts,2));
    
        cell_struct(c).faces = zeros(1, n_verts);
    
        % Build face indices for the cell
        for i = 1:n_verts
            v1 = verts(i);
            v2 = verts(mod(i, n_verts) + 1);
    
            % Enforce same ordering as before for key
            if v2 < v1
                tmp = v1; v1 = v2; v2 = tmp; end
    
            key1 = sprintf('%d_%d', v1, v2);
            f_id = face_map(key1);
    
            cell_struct(c).faces(i) = f_id;
        end
    
        % Compute faces_orientation based on vertex ordering (original)
        edge_list = cell(1, n_verts);
        for i = 1:n_verts
            v1 = verts(i);
            v2 = verts(mod(i, n_verts) + 1);
            edge_list{i} = [v1, v2];
        end
        faces_orientation = cellfun(@(e) sign(e(2) - e(1)), edge_list);
    
        cell_struct(c).faces_orientation = faces_orientation;
    end
end
