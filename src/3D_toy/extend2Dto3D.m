function [cell3D, face3D, V3, cells3D] = extend2Dto3D(V2, cells2D, H)
    ...
    % --- Global face map: key = sorted vertex ids ---
    face_map   = containers.Map('KeyType','char','ValueType','int32');
    face_verts = {};              % face_verts{fid} = vertex ids (MATLAB 1-based)
    face_cells = {};              % face_cells{fid} = adjacent cell ids
    ...
    % helper to register face (order-insensitive unique)
        function fid = registerFace(vlist, cell_id)
            ...
            if isKey(face_map, key)
                ...
                face_map(key) = fid;
                face_verts{fid} = vlist(:)';  % keep an orientation (first time)
    ...
    for c = 1:nCells
        ...
        % top face (polygon) - use same vertex order as bottom but shifted
    ...
    for f = 1:nFaces
       ...
       %%%%%% caution!!!
        % % normal from polygon/quad: triangulate with first vertex
        % nsum = [0 0 0];
        ...
        face3D(f).center = fc(:);
        face3D(f).normal = n_unit(:);      % global normal (as registered first time)
        face3D(f).area   = area;
